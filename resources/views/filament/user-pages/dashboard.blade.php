...
<x-filament::page>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div class="bg-white rounded-lg shadow p-6 flex flex-col items-center">
            <x-heroicon-o-cake class="w-10 h-10 text-primary-500 mb-2" />
            <div class="text-3xl font-bold">{{ $totalRecipes }}</div>
            <div class="text-gray-600">Tổng số công thức</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6 flex flex-col items-center">
            <x-heroicon-o-document-text class="w-10 h-10 text-blue-500 mb-2" />
            <div class="text-3xl font-bold">{{ $totalPosts }}</div>
            <div class="text-gray-600">Tổng số bài viết</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center gap-2 mb-2">
                <x-heroicon-o-cake class="w-6 h-6 text-primary-500" />
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="font-semibold">Bài viết mới nhất</span>
            </div>
            @if($latestPost)
                <div>
                    <div class="font-bold text-lg">{{ $latestPost->title }}</div>
                    <div class="text-gray-500 text-sm mb-1">Tạo lúc: {{ $latestPost->created_at->format('d/m/Y H:i') }}</div>
                    <div class="line-clamp-2 text-gray-700">{{ $latestPost->excerpt }}</div>
                    <a href="{{ route('filament.user.resources.user-posts.edit', $latestPost->id) }}" class="inline-block mt-2 text-blue-600 hover:underline text-sm">Chỉnh sửa</a>
                </div>
            @else
                <div class="text-gray-400">Bạn chưa có bài viết nào.</div>
            @endif
        </div>
    </div>
</x-filament::page> 