        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Chỉnh sửa công thức</h1>
                        <p class="text-gray-600 mt-1">Cập nhật thông tin công thức "{{ $recipe->title }}"</p>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <select id="categories" 
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" 
                                                    {{ in_array($category->id, old('categories', $recipe->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select id="tags" 
                                        ...
                                        @foreach($tags as $tag)
                                            <option value="{{ $tag->id }}" 
                                                    {{ in_array($tag->id, old('tags', $recipe->tags->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                #{{ $tag->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    @foreach($recipe->instructions as $index => $instruction)
                                        <div class="instruction-item grid grid-cols-1 md:grid-cols-6 gap-3 mb-3">
                                            ...
                                            <div class="md:col-span-4">
                                                <textarea name="instructions[{{ $index }}][instruction]" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    @if($recipe->cover_image)
                                        <div class="mt-2">
                                            <img src="{{ Storage::url($recipe->cover_image) }}" 
                                                 alt="Ảnh bìa hiện tại" 
                                                 class="w-32 h-24 object-cover rounded">
                                            <p class="text-sm text-gray-500 mt-1">Ảnh bìa hiện tại</p>
                                        </div>
                                    @endif
                                    @error('cover_image')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
...
<script>
let ingredientCount = {{ $recipe->ingredients ? count($recipe->ingredients) : 1 }};
let instructionCount = {{ $recipe->instructions ? count($recipe->instructions) : 1 }};
